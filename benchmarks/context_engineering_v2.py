#!/usr/bin/env python3
"""Context Engineering Pipeline: Extract → Store → Retrieve knowledge."""
...
# Step 1: Generate corpus with REAL knowledge
print("\n  [1/6] Generating corpus with real knowledge...")
...
for concept, facts in corpus_pages.items():
    for fact in facts:
        ...
        corpus.extend([f"filler_{concept}_{i}" for i in range(20)])
...
for query in queries:
    ...
    if result.pages:
        print(f"    Preview: {result.pages[0][:150]}...")
...
print(f"\n  --- SUMMARY ---")
print(f"  Corpus: {len(corpus):,} tokens")
print(f"  Memory: {mem/1024/1024:.2f} MB ({mem/len(corpus):.4f} bytes/token)")
